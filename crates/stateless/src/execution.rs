...
impl From<StatelessValidationError> for StatelessExecutionError {
    fn from(error: StatelessValidationError) -> Self {
        match error {
            StatelessValidationError::Custom(msg) => StatelessExecutionError::Custom(msg),
            _ => StatelessExecutionError::StatelessExecutionFailed(error.to_string()),
        }
    }
}
...
pub fn stateless_execution<ChainSpec, E>(
    ...
    evm_config: E,
) -> Result<(B256, BlockExecutionOutput<EthereumReceipt>), StatelessExecutionError>
where
    ...
    E: ConfigureEvm<Primitives = EthPrimitives> + Clone + 'static,
{
    ...
    if count > BLOCKHASH_ANCESTOR_LIMIT {
        return Err(StatelessExecutionError::AncestorHeaderLimitExceeded {
            count,
