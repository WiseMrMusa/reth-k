...
/// `current_block`.
pub fn stateless_execution<ChainSpec, E>(
    current_block: Block,
    witness: ExecutionWitness,
    chain_spec: Arc<ChainSpec>,
    evm_config: E,
) -> Result<B256, StatelessExecutionError>
where
    ChainSpec: Send + Sync + EthChainSpec<Header = Header> + EthereumHardforks + Debug,
    E: ConfigureEvm<Primitives = EthPrimitives> + Clone + 'static,
{
    stateless_execution_with_trie::<StatelessSparseTrie, ChainSpec, E>(
        current_block,
        witness,
        chain_spec,
        evm_config,
    ...
    E: ConfigureEvm<Primitives = EthPrimitives> + Clone + 'static,
{
    let current_block = current_block
        .try_into_recovered()
        .map_err(|err| StatelessExecutionError::SignerRecovery(Box::new(err)))?;
    ...
    let pre_state_root = match ancestor_headers.last() {
        ...
        None => return Err(StatelessExecutionError::MissingAncestorHeader),
    };

    // First verify that the pre-state reads are correct
    let (mut trie, bytecode) = T::new(&witness, pre_state_root).map_err(|_| StatelessExecutionError::StatelessPreStateRootCalculationFailed)?;
    ...
    let output = executor
        ...
        .map_err(|e| StatelessExecutionError::StatelessExecutionFailed(e.to_string()))?;

    // Post validation checks
    // validate_block_post_execution(&current_block, &chain_spec, &output.receipts, &output.requests)
    //     .map_err(StatelessExecutionError::ConsensusValidationFailed)?;

    // Compute and check the post state root
    // let hashed_state = HashedPostState::from_bundle_state::<KeccakKeyHasher>(&output.state.state);
    // let state_root = trie.calculate_state_root(hashed_state).map_err(|_| StatelessExecutionError::StatelessStateRootCalculationFailed)?;
    // if state_root != current_block.state_root {
    //     return Err(StatelessExecutionError::PostStateRootMismatch {
    //         got: state_root,
...
fn compute_ancestor_hashes(
...
) -> Result<BTreeMap<u64, B256>, StatelessExecutionError> {
    ...
    for parent_header in ancestor_headers.iter().rev() {
        ...
        if parent_hash != parent_header.hash_slow() {
            return Err(StatelessExecutionError::InvalidAncestorChain); // Blocks must be contiguous
        }

        if parent_header.number + 1 != child_header.number {
            return Err(StatelessExecutionError::InvalidAncestorChain); // Header number should be
                                                                        // contiguous
        }
